...
- [ ] `gitleaks` als Pre-commit Hook konfiguriert (Secrets in Commits verhindern) — `.gitleaks.toml` erstellt, gitleaks-action in CI, lokaler Hook benötigt gitleaks CLI Installation (manuell)
- [x] `truffleHog` in CI-Workflow (GitHub Actions) integriert
- [ ] Socket.dev GitHub App installiert (Supply Chain Security) — manuelle Aktion, dokumentiert
...
## Offene Punkte
- **Socket.dev**: GitHub App muss manuell von Andi installiert werden
- **gitleaks lokal**: Für lokalen Pre-commit Hook muss `gitleaks` CLI separat installiert werden (CI reicht für MVP)
- **Warnings reduzieren**: Die ~100 Lint-Warnings sind ein Backlog für zukünftige Refactoring-Tickets
