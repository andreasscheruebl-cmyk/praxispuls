<#
.SYNOPSIS
    TicketOps - Visuelles Kanban Board im Browser
.DESCRIPTION
    Liest alle Tickets aus .tickets/ und generiert ein interaktives HTML-Board.
    Usage: .\ticket-board-visual.ps1
#>
...
foreach ($status in "backlog","active","review","done") {
    ...
    foreach ($f in $files) {
        ...
        $allTickets += [PSCustomObject]@{
            ...
            Priority = Read-Field $c "priority"
            Sprint   = $sprint
            Branch   = Read-Field $c "branch"
            Created  = Read-Field $c "created"
            Updated  = Read-Field $c "updated"
            Estimate = Read-Field $c "estimate"
            Desc     = $desc
            File     = $f.Name
        }
    }
...
$jsonLines = "const boardData = {"
foreach ($status in "backlog","active","review","done") {
    $jsonLines += "`n  $status`: ["
    $tickets = $allTickets | Where-Object { $_.Status -eq $status }
    foreach ($t in $tickets) {
        $jsonLines += "`n    {id:'$(Esc $t.Id)',title:'$(Esc $t.Title)',type:'$(Esc $t.Type)',priority:'$(Esc $t.Priority)',sprint:'$(Esc $t.Sprint)',branch:'$(Esc $t.Branch)',created:'$(Esc $t.Created)',updated:'$(Esc $t.Updated)',estimate:'$(Esc $t.Estimate)',desc:'$(Esc $t.Desc)',file:'$(Esc $t.File)'},"
    }
    $jsonLines += "`n  ],"
}
$jsonLines += "`n};"

$sprintList = ($sprintSet.Keys | Sort-Object | ForEach-Object { "'$_'" }) -join ","
$sprintJs = "const sprints = [$sprintList];"
...
$htmlContent = $htmlContent.Replace('__BOARD_DATA__', $jsonLines)
$htmlContent = $htmlContent.Replace('__SPRINT_DATA__', $sprintJs)

# ── Datei schreiben + Browser oeffnen ───────────────────────
$outputPath = Join-Path (Get-Location) ".tickets/board.html"
Set-Content -Path $outputPath -Value $htmlContent -Encoding UTF8

Write-Host ""
Write-Host "  Board generiert: .tickets/board.html" -ForegroundColor Green
Write-Host "  $totalCount Tickets geladen" -ForegroundColor Cyan
